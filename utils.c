...
void parse_args(int argc, char **argv, ...)
{
    ...
    while((pattern = va_arg(l, char*))){
        ...
        if(print_help){
            ...
            else if(pos && !strcmp(p, "%f"))
                printf(" / %g", *(float *)pointer);
            else if(pos && !strcmp(p, "%lf"))
                printf(" / %g", *(double *)pointer);
            else if(pos && !strcmp(p, "%Ld"))
                printf(" / %Ld", *(long long int *)pointer);
            else if(pos && !strcmp(p, "%s") && *(char **)pointer)
                printf(" / %s", *(char **)pointer);
            else if(!pos && !strcmp(pattern, "%s") && *(char **)pointer)
                printf(" / %s", *(char **)pointer);
            printf("\n");
        } else if(pos){
...
u_int64_t free_disk_space(char *path)
{
    ...
    u_int64_t free = 0;
    u_int64_t total = 0;
    u_int64_t total_free = 0;

    GetDiskFreeSpaceEx(path, (PULARGE_INTEGER)&free, (PULARGE_INTEGER)&total, (PULARGE_INTEGER)&total_free);
